...
export async function embedAsset(request:Request,env:Env,token:string,assetId:string,rid:string):Promise<Response>{
  ...
  const branding=safeObject(embed.branding_json),canDownload=branding.downloads===true&&row.downloadable===1,fileId=variant?row.variant_drive_file_id:row.original_drive_file_id,mime=(variant?row.variant_mime_type:row.mime_type)||'application/octet-stream';
...
export async function embedViewer(request:Request,env:Env,token:string,assetId:string,rid:string):Promise<Response>{
  ...
  const related=await env.DB.prepare(`SELECT a.id,a.type,a.title,c.captured_at FROM assets a LEFT JOIN captures c ON c.id=a.capture_id WHERE a.project_id=?1 AND a.status='published' ORDER BY c.captured_at DESC,a.created_at DESC LIMIT 200`).bind(embed.project_id).all<{id:string;type:string;title:string;captured_at:string|null}>();
  const visibleRelated=related.results.filter(item=>!allowed.length||allowed.includes(item.id)||allowed.includes(item.type));
